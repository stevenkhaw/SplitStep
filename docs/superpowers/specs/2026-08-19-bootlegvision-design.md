...
**Backend** — Python 3.12 (pinned; 3.13 torch wheels are still unreliable), FastAPI, uvicorn, stdlib `sqlite3` with explicit SQL, ultralytics (YOLO11), opencv-python-headless, ffmpeg via subprocess.
...
/Volumes/BootlegVision/
  ...
  sessions/<id>/
    ...
    features.jsonl                 per-frame detection features
...
**Reclaim Space** is a button, not a policy. It deletes `original.mov` and sets `has_original = 0`. It refuses to run if any starred rally in the session lacks an exported clip — otherwise it would destroy the only 4K source for a rally that was explicitly marked worth keeping. Proxy and clips are never touched, so a reclaimed session stays fully browsable and re-editable at 1080p forever.
...
CREATE TABLE sessions (
  id              TEXT PRIMARY KEY,        -- '2026-08-19-morning'
  title           TEXT NOT NULL,
  ...
  status          TEXT NOT NULL,           -- ingesting|ingested|detecting|ready|reviewed|failed
...
CREATE TABLE rallies (
  ...
  idx           INTEGER NOT NULL,          -- 1-based order within session
  start_ms      INTEGER NOT NULL,          -- current bounds, possibly trimmed by hand
...
4. **Drop segments < 3 s** — a serve and a netted return is still ~4 s
5. **Pad −1.0 s / +1.5 s** — catch the toss, let the point breathe before the cut
...
**Warmup is indistinguishable from a rally.** Both players present, both near their baselines, both moving. There is no cheap signal separating them. Not solved: the first few minutes of every session produce rejects that take about twenty seconds to clear in queue mode. A "session starts at ⟨time⟩" trim control is the fallback if it becomes annoying.
...
This is what makes queue mode work at all. A single element seeking between rallies stalls roughly 400 ms on every advance, and gaps between points can be a minute or more. Preloaded, advance is instant and the review rhythm never breaks.
...
1. **Overview** — the whole session, every rally a block, starred ones highlighted. For orientation and jumping. A rally is about 3 px wide here, which is fine because no editing happens at this scale.
...
Then: create the session row, move the file into `sessions/<id>/`, enqueue `ingest`.
...
| Audio ignored | Windy courts destroy ball-impact transients |
...
1. **Segmentation weights** (`w1`–`w4`) and the threshold are unset. They require a labeled session of real footage. First implementation milestone should be: ingest one session, hand-label rallies, fit the weights.
