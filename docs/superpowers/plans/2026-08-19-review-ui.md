# BootlegVision Review UI — Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-08-19-bootlegvision-design.md` §6
...
Python helpers: `bootleg/db/rallies.py::mark_reviewed`, `bootleg/detect/features.py::read_features`, `bootleg/detect/segment.py::score_series`, `bootleg/media/transcode.py::run_ffmpeg`, `bootleg/detect/geometry.py::Quad`.
...
```
bootleg/api/routes.py        modify — 5 new endpoints
bootleg/api/spa.py           new — mount the built SPA, SPA-fallback for client routes
bootleg/media/frames.py      new — extract a single frame as JPEG
...
**Interfaces:**
- Consumes: `mark_reviewed(conn, rally_id)` from `bootleg/db/rallies.py`; `set_session_status(conn, session_id, status)` from `bootleg/db/sessions.py`
- Produces: `POST /api/rallies/{rally_id}/reviewed` → `{"ok": true, "session_status": str}`; `refresh_session_review_status(conn, session_id) -> str` in `bootleg/db/sessions.py`
...
Run: `~/miniconda3/envs/bootleg/bin/pytest tests/test_api_review.py -v`
Expected: FAIL — 404 on the new route, `AssertionError` on status transitions.

- [ ] **Step 3: Add `refresh_session_review_status` to `bootleg/db/sessions.py`**
...
In `bootleg/api/routes.py`, add the import and the endpoint:

```python
from bootleg.db.rallies import mark_reviewed
from bootleg.db.sessions import refresh_session_review_status
...
```bash
git add bootleg/api/routes.py bootleg/db/sessions.py tests/test_api_review.py
...
**Interfaces:**
- Consumes: `read_features` from `bootleg/detect/features.py`, `score_series` and `SegmentParams` from `bootleg/detect/segment.py`
...
Run: `~/miniconda3/envs/bootleg/bin/pytest tests/test_api_review.py -k scores -v`
Expected: FAIL — 404, route does not exist.

- [ ] **Step 3: Add the route**

In `bootleg/api/routes.py`:

```python
from bootleg.detect.segment import score_series
...
**Files:**
- Create: `bootleg/media/frames.py`
- Modify: `bootleg/api/routes.py`
- Test: `tests/test_frames.py`
- Test: `tests/test_api_review.py`

**Interfaces:**
- Consumes: `run_ffmpeg` from `bootleg/media/transcode.py`; `Quad` from `bootleg/detect/geometry.py`
- Produces: `extract_frame(src: Path, dst: Path, at_ms: int = 0, width: int = 1280) -> None` in `bootleg/media/frames.py`; `GET /media/{session_id}/{idx}/frame.jpg`; `GET /api/court_presets`; `POST /api/court_presets` body `{"name": str, "points": [[x,y] x4]}` → `{"id": str}`
...
Run: `~/miniconda3/envs/bootleg/bin/pytest tests/test_frames.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'bootleg.media.frames'`

- [ ] **Step 3: Write `bootleg/media/frames.py`**
...
```bash
git add bootleg/media/frames.py bootleg/api/routes.py tests/test_frames.py tests/test_api_review.py
...
**Files:**
- Create: `bootleg/api/spa.py`
- Modify: `bootleg/api/app.py`
- Test: `tests/test_api_review.py`

**Interfaces:**
- Consumes: nothing
- Produces: `mount_spa(app: FastAPI, dist: Path) -> None` in `bootleg/api/spa.py`
...
Run: `~/miniconda3/envs/bootleg/bin/pytest tests/test_api_review.py -k spa -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'bootleg.api.spa'`

- [ ] **Step 3: Write `bootleg/api/spa.py`**
...
- [ ] **Step 4: Wire it in `bootleg/api/app.py`**
...
In `bootleg/cli.py`, `cmd_serve` passes the built bundle:
...
```bash
git add bootleg/api/spa.py bootleg/api/app.py bootleg/cli.py tests/test_api_review.py
...
```bash
bootleg --library /Volumes/BootlegVision serve
```
```bash
cd web && npm run dev
```
...
cd .. && bootleg --library /tmp/bl serve
```
...
cd web
npm install
npm run build          # once; output goes to web/dist and is served by `bootleg serve`
```

Then `bootleg --library /Volumes/BootlegVision serve` and open http://127.0.0.1:8420.
...
```bash
bootleg --library /Volumes/BootlegVision serve   # terminal 1
